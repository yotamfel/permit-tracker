...
from app.models.user_file import ALLOWED_CONTENT_TYPES, MAX_FILE_SIZE_BYTES, UserFile
from app.schemas.user_file import UserFileOut
...
def _validate_attachment_target(
    ...
    if checklist_item_id is not None and user_checklist_item_id is not None:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "A file can attach to at most one checklist row")
...
def list_files(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> list[UserFileOut]:
    ...
    return [UserFileOut.model_validate(f) for f in files]
...
async def upload_file(
    ...
    db.commit()
    db.refresh(record)
    return UserFileOut.model_validate(record)
