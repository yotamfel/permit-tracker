...
- `application_url` — the actionable "apply/book here" link shown to users after they unlock the destination. Only set this if you find a real, verified, individual-facing link. **Never point to one specific commercial tour operator when many exist** — stay neutral, prefer official government/park/booking portals. If several operators are commonly used and there's no single official booking page, leave `application_url` null and instead add one `DestinationOperator` row per legitimate operator (`destination_id`, `name`, `url` if verified live, `phone`, `email`, `note`, `order_index`) — rendered to owners as a plain neutral contact list on the destination page. Don't just name them in checklist prose anymore; use the table. **Every time this situation comes up, look for and fill in `phone` and `email` too, not just `name`/`url`** — check the operator's official site (usually a "Contact" page) for a real phone number and email address. Leave a field null rather than guess if you can't find/verify it, same as any other fact.
...
`description` (via translation, `destination.description`): public, shown free before purchase. ~4-6 sentences. What the place/activity is, what's special about it, enough texture for someone browsing without a specific target in mind. Do **not** include mechanism/pricing/prep details here — that's what `mechanism_explanation` and the checklist are for. **Write it as 2-4 paragraphs, separated by a blank line (`\n\n`), not one unbroken block** — break at genuine topic shifts (e.g. what it is/what's special about it → logistics or access → any permit/park context relevant at the description level). The frontend renders each `\n`-separated chunk as its own `<p>`, so a single block reads as one dense wall of text.

`mechanism_explanation` (via translation, `destination.mechanism_explanation`): the "how it works" technical explanation. Concise, clearly worded, minimal unnecessary text — the opposite style from `description`. This is gated behind purchase server-side, so don't worry about it being too terse for a casual browser — the reader has already paid and wants the facts fast. **Same paragraph-break rule applies** — for anything beyond a couple of sentences, split into paragraphs (`\n\n`) at real transitions (what type of mechanism it is → when/how release or application happens → quota/competitiveness mechanics → what to do if you miss out, or whatever structure actually fits this destination). Don't force paragraph breaks on a genuinely short explanation that's already just 1-2 sentences.
...
- **פערים או אי-ודאויות** — anything you couldn't verify and left blank or approximated, and why. Be honest here even if it makes the destination look incomplete — this is exactly what the reviewer and the admin need to know. **If the uncertain value came from some source (even a weak/secondary one), include that source's actual URL inline here** — not just "found in a secondary source" — so the admin can check it themselves without digging through `DestinationSource` rows. If two sources gave you conflicting numbers/dates/rules, state both claims with both URLs, not just the one you picked. Include any contact email/phone/contact-form you came across that's relevant to resolving the gap, even if you didn't use it.
