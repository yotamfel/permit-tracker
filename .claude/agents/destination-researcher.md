...
- `application_url` — the actionable "apply/book here" link shown to users after they unlock the destination. Only set this if you find a real, verified, individual-facing link. **Never point to one specific commercial tour operator when many exist** — stay neutral, prefer official government/park/booking portals. If several operators are commonly used and there's no single official booking page, leave `application_url` null and instead add one `DestinationOperator` row per legitimate operator (`destination_id`, `name`, `url` if verified live, `note`, `order_index`) — rendered to owners as a plain neutral contact list on the destination page. Don't just name them in checklist prose anymore; use the table.
