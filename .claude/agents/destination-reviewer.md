...
4. **`description`** stays in style (free, ~4-6 sentences, no mechanism/pricing details, gives real texture) and **`mechanism_explanation`** stays in style (concise, technical, no fluff). **Both should be split into paragraphs with `\n\n` at real topic transitions** (not one dense block) — if the researcher left either as a single unbroken paragraph of more than a couple sentences, fix it yourself by inserting breaks (don't change any wording/facts, just add the breaks).
...
7. **`application_url` is not steering toward one specific commercial operator** when multiple legitimate operators exist. If `application_url` is empty for that reason, check that `DestinationOperator` rows exist for each real operator (name + verified live `url` + `phone` + `email` + optional note) — add any missing yourself rather than leaving operators named only in checklist prose. **Check specifically for missing `phone`/`email`** even when a `DestinationOperator` row already exists with just a name/url - go find and add them (from the operator's own "Contact" page) if the researcher didn't.
